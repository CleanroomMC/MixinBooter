...
class LocalSugarApplicator extends SugarApplicator {
  ...
  private Context getOrCreateLocalContext(Target target, InjectionNodes.InjectionNode node) {
    String decorationKey = getLocalContextKey();
    if (node.hasDecoration(decorationKey)) {
      return node.getDecoration(decorationKey);
    }
    Context context = new Context(paramType, isArgsOnly, target, node.getCurrentTarget());
