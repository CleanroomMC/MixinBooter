...
import zone.rong.mixinextras.sugar.SugarBridge;
import zone.rong.mixinextras.sugar.impl.handlers.HandlerInfo;
import zone.rong.mixinextras.utils.GenericParamParser;
...
public class SugarWrapperInjectionInfo extends InjectionInfo implements LateApplyingInjectorInfo {
    ...
    public void prepare() {
        delegate.prepare();
        handler.visibleAnnotations.remove(InjectionInfo.getInjectorAnnotation(this.getContext().getMixin(), handler));
    ...
    private MethodNode prepareHandler(MethodNode original) {
        IMixinInfo mixin = this.getContext().getMixin();
